...
Si le Gateway s'exécute en tant que daemon (launchd/systemd), assurez-vous que `CLOUDFLARE_AI_GATEWAY_API_KEY` est accessible par ce processus (par exemple, dans `~/.openclaw/.env` ou via `env.shellEnv`).
