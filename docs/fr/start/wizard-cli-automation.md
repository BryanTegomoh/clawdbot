---
summary: "Intégration scriptée et configuration d'agent pour le CLI OpenClaw"
read_when:
...
Utilisez `--secret-input-mode ref` pour stocker les références basées sur env dans les profils d'authentification au lieu de valeurs en texte clair.
...
En mode `ref` non interactif, les variables env du fournisseur doivent être définies dans l'environnement du processus.
Passer des drapeaux de clé en ligne sans la variable env correspondante échoue maintenant rapidement.
...
- Référence complète : [Référence d'intégration CLI](/start/wizard-cli-reference)
