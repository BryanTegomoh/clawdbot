---
summary: "Fonctionnement des scripts d'installation (install.sh, install-cli.sh, install.ps1), drapeaux et automatisation"
read_when:
...
title: "Détails du script d'installation"
x-i18n:
...
# Détails du script d'installation
...
<Note>
Si l'installation réussit mais que `openclaw` est introuvable dans un nouveau terminal, voir [Dépannage Node.js](/install/node#troubleshooting).
</Note>
...
<Steps>
  <Step title="Détecter l'OS">
    Supporte macOS et Linux (y compris WSL). Si macOS est détecté, installe Homebrew s'il est absent.
  </Step>
  ...
  <Step title="Tâches post-installation">
    ...
    - Tente la configuration initiale lorsque c'est approprié (TTY disponible, configuration initiale non désactivée, et vérifications de bootstrap/config réussies)
...
| `--no-git-update`               | Ignorer le `git pull` pour un checkout existant            |
| `--no-prompt`                   | Désactiver les prompts                                     |
...
| `--verbose`                     | Activer la sortie de débogage (`set -x`, journaux npm)     |
...
| `OPENCLAW_NO_PROMPT=1`                      | Désactiver les prompts                        |
...
<Info>
Conçu pour les environnements où vous voulez tout sous un préfixe local (par défaut `~/.openclaw`) sans dépendance système à Node.
</Info>
...
<Tabs>
  ...
  <Tab title="Lancer la configuration initiale">
    ```bash
...
| `--onboard`            | Lancer `openclaw onboard` après l'installation                                  |
...
| `OPENCLAW_NO_ONBOARD=1`                     | Ignorer la configuration initiale                                                 |
| `OPENCLAW_NPM_LOGLEVEL=error\|warn\|notice` | Niveau de journalisation npm                                                      |
| `OPENCLAW_GIT_DIR=<path>`                   | Chemin de recherche pour nettoyage legacy (utilisé pour supprimer l'ancien checkout du sous-module `Peekaboo`) |
| `SHARP_IGNORE_GLOBAL_LIBVIPS=0\|1`          | Contrôler le comportement sharp/libvips (par défaut : `1`)                        |
...
<Steps>
  ...
  <Step title="Assurer Node.js 22+">
    Si absent, tente l'installation via winget, puis Chocolatey, puis Scoop.
  </Step>
  <Step title="Installer OpenClaw">
  ...
  <Step title="Tâches post-installation">
    Ajouté le répertoire bin nécessaire au PATH utilisateur lorsque possible, puis exécute `openclaw doctor --non-interactive` lors des mises à jour et installations git (au mieux).
  </Step>
</Steps>
...
| `-NoGitUpdate`            | Ignorer le `git pull`                                  |
...
<AccordionGroup>
  <Accordion title="Pourquoi Git est-il requis ?">
    Git est requis pour la méthode d'installation `git`. Pour les installations `npm`, Git est toujours vérifié/installé pour éviter les échecs `spawn git ENOENT` quand les dépendances utilisent des URLs git.
  </Accordion>

  <Accordion title="Pourquoi npm rencontre EACCES sur Linux ?">
    Certaines configurations Linux pointent le préfixe global npm vers des chemins appartenant à root. `install.sh` peut basculer le préfixe vers `~/.npm-global` et ajouter les exports PATH aux fichiers rc du shell (quand ces fichiers existent).
  </Accordion>

  <Accordion title="Problèmes sharp/libvips">
    Les scripts définissent par défaut `SHARP_IGNORE_GLOBAL_LIBVIPS=1` pour éviter que sharp compile contre le libvips système. Pour remplacer :
  ...
  <Accordion title="Windows : comment obtenir une sortie détaillée de l'installateur">
    `install.ps1` n'expose pas encore de switch `-Verbose`.
